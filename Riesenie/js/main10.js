...
function init() {
	...
	document.addEventListener("onmousedown", onDocumentMouseDown);
...
function onDocumentMouseDown( e )
    {
        ...
        if(intersects.length > 0) {
            ...
            selectedColor =  selectedObject.material.color.getHex();

            selectedObject.material.color.setRGB( 1, 1, 1 );
...
function main(){
	...
	for (i = 0; i < Object.keys(datasetData).length; i++){
		...
		result = createText([current_x, current_y, current_z], key,
		 matFront,
		  new THREE.MeshPhongMaterial( { color: 0xcc0000 }), textSize);
